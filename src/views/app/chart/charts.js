...
export const doughnutChartFormat = (data, numberOfChoice) => {
  return {
    labels: data.map(p => p.value),
    datasets: [
      {
        borderColor: data.map((p, index) => colors['themeColor'+(index+1)]),
        backgroundColor: data.map((p, index) => colors['themeColor'+(index+1)+'_10']),
        borderWidth: 2,
        data: data.map(p => p.numberResponses)
      }
    ]
  }
}
